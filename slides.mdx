...
*
1:5 > { "my_var": "foobar" }
6:9 > { "my_var": "VARIABLE IS NOT DEFINED!" }
```
...
----
*
3:5 > define variable inside playbook
7:10 > ensure parsed template on remote
```
...
----
*
3,4 > variable from playbook
6 > variable from facts
```
...
----
*
6:8 > inline style for package related modules
15:18 > regular style with loop/item
```
...
----
*
4:11 > Disable root login in ssh config file
12:17 > enable the ssh service on startup
19 > Start handlers section (same indention level as "tasks")
20:24 > restart ssh service
11, 17, 20 > notify the task inside handlers
```
